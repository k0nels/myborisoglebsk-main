...
// Изначальная загрузка всех новостей
if ($category_id > 0) {
    $stmt = $pdo->prepare("SELECT news.*, categories.name AS category_name FROM news JOIN categories ON news.category_id = categories.id WHERE categories.id = ? ORDER BY news.created_at DESC");
    $stmt->execute([$category_id]);
} else {
    $stmt = $pdo->query("SELECT news.*, categories.name AS category_name FROM news JOIN categories ON news.category_id = categories.id ORDER BY news.created_at DESC");
}
$newsList = $stmt->fetchAll();
$categories = $pdo->query("SELECT * FROM categories ORDER BY id")->fetchAll();

$pageTitle = ($category_id > 0 && !empty($newsList)) ? "Новости: " . $newsList[0]['category_name'] : "Все новости Борисоглебска";
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<li><a class="dropdown-item" href="auth_handler.php?action=logout"><i class="bi bi-box-arrow-left"></i> Выйти</a></li>
    ...
    <main class="py-5">
        <div class="container">
            ...
            <nav aria-label="breadcrumb" class="mb-4" data-aos="fade-up">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="index.php" style="color: var(--primary-purple);">Главная</a></li>
                    <li class="breadcrumb-item active"><?= $pageTitle ?></li>
                </ol>
                <h1 class="fw-bold display-5"><?= $pageTitle ?></h1>
            </nav>

            <!-- Фильтр (Табы) -->
            <div class="tabs-pills mb-5" id="news-tabs" data-aos="fade-up">
                <a href="fetch_data.php?category=0&full=1" class="nav-link <?= $category_id == 0 ? 'active' : '' ?>">Все новости</a>
                <?php foreach($categories as $cat): ?>
                    <?php if(mb_strtolower($cat['name']) == 'все новости') continue; ?>
                    <a href="fetch_data.php?category=<?= $cat['id'] ?>&full=1" class="nav-link <?= $category_id == $cat['id'] ? 'active' : '' ?>">
                        <?= htmlspecialchars($cat['name']) ?>
                    </a>
                <?php endforeach; ?>
            </div>

            <!-- Контейнер новостей (3 колонки) -->
                ...
                <div class="col-md-4 mb-4" data-aos="fade-up">
                    <a href="news.php?id=<?= $news['id'] ?>" class="news-card h-100">
                        <div class="news-card-img">
                            <?php 
                                $img = $news['image'];
                                $src = (empty($img)) ? "https://via.placeholder.com/600x400" : ((strpos($img, 'http') === 0) ? $img : "img/".$img);
                            ?>
                            <img src="<?= $src ?>" alt="" onerror="this.src='https://via.placeholder.com/600x400'">
                        </div>
                        <div class="news-card-body">
                            ...
                            <div class="news-card-meta">
                                ...
                                <span><i class="bi bi-eye"></i> <?= rand(100, 3000) ?></span>
                            </div>
                        </div>
                    </a>
                </div>
  ...
  <!-- FOOTER -->
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const newsTabs = document.querySelectorAll('#news-tabs .nav-link');
        ...
        newsTabs.forEach(tab => {
            tab.addEventListener('click', function(e) {
                ...
                // Делаем AJAX запрос к fetch_data.php?category=ID&full=1
